...
import {acknowledge,diff,overlay,validCities,validTimes,type VisitTimes,type Queue} from '@/lib/cloud-state';
const ALLOWED=new Set(cities.map(c=>c.id));
const GUEST='shanhe-cities-v2';
const GUEST_TIMES='shanhe-city-times-v1';
...
export function useFootprints(){
 const [visitTimes,setVisitTimes]=useState<VisitTimes>({});const times=useRef<VisitTimes>({});
 ...
 function show(next:string[],dates:VisitTimes=times.current){current.current=next;times.current=validTimes(dates,next);setVisitTimes(times.current);setVisited(next);}
 function persist(){try{if(owner.current)localStorage.setItem(cacheKey(owner.current),JSON.stringify({visited:current.current,queue:queue.current,visitTimes:times.current}));else {localStorage.setItem(GUEST,JSON.stringify(current.current));localStorage.setItem(GUEST_TIMES,JSON.stringify(times.current));}setStorageError(false);}catch{setStorageError(true);}setPendingCount(Object.keys(queue.current).length);}
 async function refresh(){const db=client.current,uid=owner.current,g=generation.current;if(!db||!uid)return;const serial=++refreshSerial.current;
  const {data,error}=await db.from('city_visits').select('city_id,visited,visited_at').eq('user_id',uid);
  ...
  const remote=(data||[]).filter(r=>r.visited&&ALLOWED.has(r.city_id)).map(r=>r.city_id as string);const dates:VisitTimes=Object.fromEntries((data||[]).map(r=>[r.city_id,r.visited_at]));for(const [id,c] of Object.entries(queue.current))if(c.visitedAt!==undefined)dates[id]=c.visitedAt;show(overlay(remote,queue.current),dates);persist();setCloudError('');
 ...
 async function flush(){const db=client.current,uid=owner.current,g=generation.current;if(!db||!uid||running.current===g)return;
  ...
  try{while(alive.current&&g===generation.current&&Object.keys(queue.current).length){const sent={...queue.current};++refreshSerial.current;
   const rows=Object.entries(sent).map(([city_id,c])=>({user_id:uid,city_id,visited:c.visited,visited_at:c.visitedAt??null}));
 ...
 function save(next:string[],overrides:VisitTimes={}){if(!ready)return;const clean=validCities(next,ALLOWED);const now=new Date().toISOString();const dates:VisitTimes={};for(const id of clean)dates[id]=Object.hasOwn(overrides,id)?overrides[id]:current.current.includes(id)?times.current[id]??null:now;
 const changes=diff(current.current,clean,++revision.current);for(const id of clean)if(dates[id]!==times.current[id])changes[id]={visited:true,revision:revision.current};for(const [id,c] of Object.entries(changes))c.visitedAt=dates[id]??null;
 if(owner.current){queue.current={...queue.current,...changes};show(clean,dates);persist();setStatus('pending');void flush();}else{show(clean,dates);persist();setGuestCount(clean.length);setStatus(client.current?'local':'unconfigured');}}
 function editVisitTime(id:string,value:string){if(!current.current.includes(id)||!Number.isFinite(Date.parse(value)))return;save(current.current,{[id]:new Date(value).toISOString()});}
 function guestTimes(ids:string[]){try{return validTimes(JSON.parse(localStorage.getItem(GUEST_TIMES)||'{}'),ids);}catch{return validTimes({},ids);}}
 function activate(nextUser:User|null){if(!alive.current)return;if(initialized.current&&owner.current===(nextUser?.id||null))return;initialized.current=true;generation.current++;refreshSerial.current++;owner.current=nextUser?.id||null;setUser(nextUser);setCloudError('');queue.current={};const guest=readGuest();setGuestCount(guest.length);
  if(!nextUser){show(guest,guestTimes(guest));setPendingCount(0);setReady(true);setStatus(client.current?'local':'unconfigured');return;}
  let saved:string[]=[],savedTimes:VisitTimes={};try{const raw=JSON.parse(localStorage.getItem(cacheKey(nextUser.id))||'{}');if(raw.visited)saved=validCities(raw.visited,ALLOWED);savedTimes=validTimes(raw.visitTimes,saved);if(raw.queue&&typeof raw.queue==='object')for(const [id,c] of Object.entries(raw.queue) as [string,{visited:unknown;revision:unknown;visitedAt?:string|null}][]){if(ALLOWED.has(id)&&c&&typeof c.visited==='boolean'&&typeof c.revision==='number'){queue.current[id]={visited:c.visited,revision:c.revision,visitedAt:validTimes({[id]:c.visitedAt},[id])[id]};revision.current=Math.max(revision.current,c.revision);}}}catch{setStorageError(true);}
  for(const [id,c] of Object.entries(queue.current))if(c.visitedAt!==undefined)savedTimes[id]=c.visitedAt;show(overlay(saved,queue.current),savedTimes);setPendingCount(Object.keys(queue.current).length);setReady(true);setStatus('syncing');void refresh();
 }
 ...
 function importGuest(){const guest=readGuest();save([...new Set([...current.current,...guest])],{...guestTimes(guest),...times.current});return guest.length;}
 function importBackup(raw:unknown){if(!raw||typeof raw!=='object')throw Error('备份格式不正确');const b=raw as {format?:string;version?:number;cities?:unknown;visitTimes?:unknown};if(b.format!=='shanhe-footprints'||![1,2].includes(b.version||0))throw Error('不是支持的山河足迹备份');const ids=validCities(b.cities,ALLOWED);save([...new Set([...current.current,...ids])],{...validTimes(b.visitTimes,ids),...times.current});return ids.length;}
 function exportBackup(){const body=JSON.stringify({format:'shanhe-footprints',version:2,visitTimes:times.current,exportedAt:new Date().toISOString(),cities:current.current},null,2);const url=URL.createObjectURL(new Blob([body],{type:'application/json'}));const a=document.createElement('a');a.href=url;a.download=`山河足迹-${new Date().toISOString().slice(0,10)}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}
 return {visited,visitTimes,editVisitTime,save,user,ready,status,storageError,cloudError,configured,pendingCount,guestCount,sendLogin,signOut,importGuest,importBackup,exportBackup,retry:flush};
}
